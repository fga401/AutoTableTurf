...
class TableTurfManager:
    ...
    def __multi_detect(self, detect_fn, sleep_time=0.1, max_loop=10):
    ...
    def __select_deck(self, deck: int) -> List[Card]:
        target = deck
        while True:
            current = self.__multi_detect(detection.deck_cursor)(debug=self.__debug)
            if current == target:
                break
            if current != -1:
    ...
    def __redraw(self, stage: Optional[Stage] = None, my_deck: Optional[List[Card]] = None, his_deck: Optional[List[Card]] = None):
        while self.__multi_detect(detection.redraw_cursor)(debug=self.__debug) == -1:
            sleep(0.5)
        ...
        while True:
            current = self.__multi_detect(detection.redraw_cursor)(debug=self.__debug)
            if current == target:
                break
    ...
    def __init_roi(self):
        while self.__multi_detect(detection.hands_cursor)(debug=self.__debug) == -1:
            sleep(0.5)
        rois, roi_width, roi_height = detection.stage_rois(self.__capture(), debug=self.__debug)
    ...
    def __get_status(self, round: int, my_deck: Optional[List[Card]] = None, his_deck: Optional[List[Card]] = None) -> Status:
        ...
        stage = self.__multi_detect(detection.stage)(rois=rois, roi_width=roi_width, roi_height=roi_height, last_stage=last_stage, debug=self.__debug)
    ...
    def __move_hands_cursor(self, target):
        while True:
            current = self.__multi_detect(detection.hands_cursor)(debug=self.__debug)
            if current == target:
                break
    ...
    def __move(self, status: Status, step: Step):
        if step.action == step.Action.Skip:
            ...
            while not self.__multi_detect(detection.skip)(debug=self.__debug):
                self.__controller.press_buttons([Controller.Button.A])
        ...
        if step.action == step.Action.SpecialAttack:
            ...
            while not self.__multi_detect(detection.special_on)(debug=self.__debug):
                self.__controller.press_buttons([Controller.Button.A])
        # select card
        self.__move_hands_cursor(status.hands.index(step.card))
        ...
        while True:
            self.__controller.press_buttons([Controller.Button.A])
            preview, current_index = self.__multi_detect(detection.preview)(stage=status.stage, rois=self.__session['rois'], roi_width=self.__session['roi_width'], roi_height=self.__session['roi_height'], debug=self.__debug)
        ...
        if step.rotate > 0:
            ...
            while True:
                actual, _ = self.__multi_detect(detection.preview)(stage=status.stage, rois=self.__session['rois'], roi_width=self.__session['roi_width'], roi_height=self.__session['roi_height'], debug=self.__debug)
